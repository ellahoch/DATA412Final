...
Procedure:
	...
	The general form of our analysis would start with the ggplot2 and dplyr libraries. We would merge the two data sets, clean the data, and eliminate different values. From there, we have several choices. We could calculate the correlation coefficient between the share of Republicans and the number of hate crimes to measure the strength and direction of the linear relationship, create a heat map to visualize the relationship between multiple variables, including the share of Republicans and hate crimes, and identify patterns visually, or categorize states based on the share of Republicans (e.g., low, medium, high) and compare the average number of hate crimes in these categories. Assuming the relationship between the two variables is found to be non-linear when calculating correlation coefficient, we use non-parametric tests like the Spearman rank correlation coefficient to assess the correlation.
